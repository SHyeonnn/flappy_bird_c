...
void Map3(int TreePosition3); // 세번째 장애물 모양 그리기
...
int main(void)
{
    ...
    int MapX3 = Distance; // 세번째 장애물의 거리를 50으로 지정
    ...
    while (true)
    {
        ...
        if (MapX2 == 38)
        {
            MapX3_Flag += 1;; // 2번째 트리의 위치가 중간에 왔다면 3번째 장애물 출력을 위한 변수 초기화
        }

        if (MapX3 <= 1)
        {
            MapX3 = Distance; // 2번째 장애물의 x좌표가 1보다 작아진다면 위치 초기화
            MapX3_Flag = 0;
        }
